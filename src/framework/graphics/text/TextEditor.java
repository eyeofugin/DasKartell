...
public class TextEditor {
    ...
    public int[] getTextLine(String text, int targetWidth, int targetHeight,
        ...
        for (char symbol : textArray) {
            ...
            if (symbolNr != text.length()) {
                ...
                lastWrittenWidth+= Property.BASE_CHAR_DISTANCE;
            }
            symbolNr++;
    ...
    public int[] getButtonLine(String text, int targetWidth, int targetHeight,
        ...
        int labelWidth = targetWidth-(Property.BUTTON_PADDING+Property.BUTTON_LINE);
        int labelHeight = targetHeight-(Property.BUTTON_PADDING+Property.BUTTON_LINE);
        ...
        for(int x = 0; x < targetWidth; x++) {
            for(int y = 0; y < Property.BUTTON_LINE; y++ ) {
                button[x+y*targetWidth] = -1;
            }
        }
        for(int x = 0; x < targetWidth; x++) {
            for(int y = targetHeight-Property.BUTTON_LINE; y < targetHeight; y++ ) {
                button[x+y*targetWidth] = -1;
            }
        }
        for(int x = 0; x < Property.BUTTON_LINE; x++) {
            for(int y = 0; y < targetHeight; y++ ) {
                button[x+y*targetWidth] = -1;
            }
        }
        for(int x = targetWidth-Property.BUTTON_LINE; x < targetWidth; x++) {
            for(int y = 0; y < targetHeight; y++ ) {
                button[x+y*targetWidth] = -1;
            }
        }
        int labelpadding = Property.BUTTON_LINE+Property.BUTTON_PADDING-2;
        int index = 0;
